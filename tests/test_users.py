...
class TestUsers:
    ...
    async def test_create_access_token_not_registered(self, test_app):
    ...
    async def test_create_access_token_registered(self, test_app):
        from uiqmako_api.api.dependencies import get_current_user
    ...
    async def test_add_user_exists(self, test_app):
        with pytest.raises(UsernameExists):
            await add_user('UserAll', 'fakepwd')

    async def test_add_user_ok(self, test_app):
    ...
    async def test_update_user_data(self, test_app):
        created = await add_user('NewUser', 'fakepwd')
